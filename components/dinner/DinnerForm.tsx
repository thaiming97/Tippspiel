...
import { submitDinnerAction } from "@/app/actions/dinner";
...
const OPTIONS: { value: DinnerVote; label: string; icon: string }[] = [
...
const CARD_STYLE: Record<DinnerVote, string> = {
...
const SEGMENT_ACTIVE: Record<DinnerVote, string> = {
...
export function DinnerForm({
  ...
  existing: Record<string, DinnerDraft>;
  disabled?: boolean;
}) {
  const [state, formAction] = useFormState(submitDinnerAction, undefined);
  const [name, setName] = useState("");
  /** Nicht beantwortete Termine bleiben leer – „Nein" ist keine Vorauswahl. */
  const [votes, setVotes] = useState<Record<string, DinnerVote>>({});
  const [picked, setPicked] = useState<string[]>([]);
  const [comment, setComment] = useState("");
  /** Name, dessen Antwort gerade geladen ist – erkennt den Bearbeiten-Fall. */
  const [loadedKey, setLoadedKey] = useState<string | null>(null);

  const months = groupByMonth(DINNER_DATES);
  const chosen = DINNER_DATES.filter((d) => votes[d] === "yes" || votes[d] === "maybe");
  ...
  function setVote(date: string, vote: DinnerVote) {
  ...
  return (
    <form action={formAction} className="space-y-6">
      ...
      <div className="card border-ink/[0.06]">
        <label className="label" htmlFor="dinner-name">
          Dein Name
        </label>
        <input
          id="dinner-name"
      ...
      <div className="card border-ink/[0.06]">
        <div className="mb-1 flex flex-wrap items-center justify-between gap-2">
          ...
          <span className="chip bg-ff-orange/10 text-ff-orange">
            {chosen.length} von {DINNER_DATES.length} gewählt
          </span>
        </div>
        ...
        {months.map((month) => (
          <div key={month.label} className="mb-5 last:mb-0">
            <div className="eyebrow mb-2.5 before:bg-ff-orange">{month.label}</div>
              ...
              {month.dates.map((date) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      role="group"
                      aria-label={`Termin ${formatDinnerDate(date)}`}
            ...
            return (
              ...
              >
                ...
                <span>
                  ...
                  <span className="block text-sm text-ink-soft">{r.town}</span>
                </span>
              </label>
            );
          })}
      ...
      {/* --- Anmerkung --- */}
      <div className="card border-ink/[0.06]">
        <label className="label" htmlFor="dinner-comment">
      ...
      <div className="flex flex-col items-center gap-2">
        ...
        <p className="text-center text-xs text-ink-soft">
          Kein Konto, keine E-Mail, kein Doodle-Abo. 🎅
        </p>
      </div>
    </form>
  );
