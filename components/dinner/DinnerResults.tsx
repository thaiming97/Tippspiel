...
import type { DinnerResponseDoc, DinnerVote } from "@/lib/types";
import { DeleteResponseForm } from "./DeleteResponseForm";

/** Darstellung einer einzelnen Zelle der Übersicht. */
const CELL: Record<DinnerVote, { className: string; icon: string; title: string }> = {
  yes: {
...
export function DinnerResults({
  ...
  const tallies = tallyDates(responses);
  const byDate = new Map(tallies.map((t) => [t.date, t]));
  const ranked = rankDates(tallies);
  const best = ranked[0];
  const restaurants = tallyRestaurants(responses);
    ...
    return (
      <div className="card border-dashed border-ink/15 bg-white/70 text-center">
        ...
        <p className="text-sm text-ink-soft">
          Sei der Erste – oder teile den Link mit der Abteilung.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="card border-ink/[0.06]">
          <div className="eyebrow mb-3 before:bg-ff-orange">Beste Termine</div>
        ...
        <div className="card border-ink/[0.06]">
          <div className="eyebrow mb-3 before:bg-ff-orange">Restaurant-Wahl</div>
          <ul className="space-y-3">
            {restaurants.map((r, i) => {
              const info = restaurantById(r.id);
              return (
                <li key={r.id}>
                  <div className="mb-1 flex items-baseline justify-between gap-2">
                    <span className="flex items-center gap-1.5 font-semibold">
                      {i === 0 && r.votes > 0 && <span aria-hidden>🍽️</span>}
                      {info?.name}{" "}
      ...
      <div className="card overflow-hidden border-ink/[0.06] p-0">
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-ink/[0.06] px-5 py-4">
          <div className="eyebrow before:bg-ff-orange">
          ...
          <table className="w-full border-collapse text-sm">
            <thead>
              <tr className="bg-ink/[0.03]">
                <th className="sticky left-0 z-10 bg-paper px-4 py-2.5 text-left font-semibold">
                  Name
                </th>
                {DINNER_DATES.map((date) => (
                  <th
                    key={date}
                    className={`whitespace-nowrap px-2 py-2.5 text-center text-xs font-semibold ${
                  ...
                  >
                    ...
                    <span className="block">{dinnerDayMonth(date)}</span>
                  </th>
                ))}
                <th className="whitespace-nowrap px-3 py-2.5 text-center text-xs font-semibold">
                  Restaurant
                </th>
                {admin && <th className="px-3 py-2.5" />}
              </tr>
              <tr className="border-b border-ink/[0.08] bg-ink/[0.03] text-xs">
                <td className="sticky left-0 z-10 bg-paper px-4 pb-2 text-ink-soft">
                  Summe
                </td>
                {DINNER_DATES.map((date) => {
            ...
            <tbody>
              {responses.map((r) => (
                ...
                >
                  <th
                    scope="row"
                    className="sticky left-0 z-10 max-w-[180px] bg-white px-4 py-2.5 text-left font-semibold"
                  >
                  ...
                  {DINNER_DATES.map((date) => {
                    ...
                    return (
                      <td
                        key={date}
                        title={cell.title}
                        className={`px-2 py-2.5 text-center ${cell.className} ${
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex justify-center gap-1">
                      {RESTAURANTS.map((option) => {
                        const on = r.restaurants.includes(option.id);
                        return (
                          <span
                            key={option.id}
                            title={`${option.name} ${option.town}${on ? "" : " – nicht gewählt"}`}
                  ...
                  {admin && (
                    <td className="px-3 py-2.5 text-right">
                      <DeleteResponseForm id={r.id} name={r.name} />
                    </td>
                  )}
                </tr>
              ))}
        ...
        <div className="border-t border-ink/[0.06] px-5 py-3 text-xs text-ink-soft">
          Restaurant-Kürzel:{" "}
          {RESTAURANTS.map((r) => `${r.short} = ${r.name} ${r.town}`).join(" · ")}
        </div>
      </div>
      ...
      {comments.length > 0 && (
        <div className="card border-ink/[0.06]">
          <div className="eyebrow mb-3 before:bg-ff-orange">Anmerkungen</div>
